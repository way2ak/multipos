...
document.addEventListener( 'DOMContentLoaded', () => {
    ...
    if ( ddwcpos( '.ddwcpos-vendors' ).length ) {
        ddwcpos( '.ddwcpos-vendors' ).select2( {
            ajax              : {
                method        : 'post',
                url           : ddwcposAdminObj.ajax.ajaxUrl,   // AJAX URL is predefined in WordPress admin
                dataType      : 'json',
                ...
                data          : params => ( {
                    query        : params.term,                      // search query
                    action       : 'ddwcpos_get_vendors_list',
                    nonce: ddwcposAdminObj.ajax.ajaxNonce,
                } ),
                processResults: response => {
                    let options = [];
                    if ( response.success && response.vendors != undefined && response.vendors != null ) {
                        ddwcpos.each( response.vendors, ( id, text ) => {
    ...
    const userRoleSelectElement = document.querySelector( 'select[name="role"]' );
    if ( userRoleSelectElement ) {
        ...
        } else if ( 'wcfm_vendor' === userRoleSelectElement.value ) {
            let template = wp.template( 'ddwcpos_vendor_pos_enabled' );
            userRoleSelectElement.closest( 'table' ).insertAdjacentHTML( 'afterend', template() );
        }
        ...
        userRoleSelectElement.addEventListener( 'change', e => {
            if ( 'ddwcpos_cashier' === e.target.value ) {
                ...
                if ( 'wcfm_vendor' === e.target.value ) {
                    let template = wp.template( 'ddwcpos_vendor_pos_enabled' );
                    e.target.closest( 'table' ).insertAdjacentHTML( 'afterend', template() );
                }
            }
        } );
    }
                ...
                if ( 'update_barcode' === productAction ) {
                    ...
                    if ( barcode ) {
                        ...
                        .then( response => {
                            ...
                            if ( response.success ) {
                                ...
                                rowElement.querySelector( 'td.column-barcode_print .ddwcpos-product-action' ).setAttribute( 'data-barcode-image', response.image );
                            }
                    ...
                    if ( barcodeQuantity ) {
                        ...
                        for ( let i = 1; i <= barcodeQuantity; i++ ) {
                            printContents += `<div style="width:max-content; margin:${ddwcposAdminObj.ddwcpos_configuration.barcode_margin}; display:inline-block; text-align: center;"><img src="${barcodeImage}" alt="Barcode" /></div>`;
                        }
