...
spl_autoload_register( 'DDWCMultiPOS\Autoload\ddwcpos_namespace_class_autoload' );

/**
 * @param string $class_name The name of the class to load.
 */
function ddwcpos_namespace_class_autoload( $class_name ) {
    if ( false === strpos( $class_name, 'DDWCMultiPOS' ) ) {
        return;
    }
    ...
    for ( $i = count( $file_parts ) - 1; $i > 0; $i-- ) {
		...
		$current = str_ireplace( 'ddwcpos-', '', $current );
    ...
    if ( file_exists( $filepath ) ) {
        ...
        wp_die(
			...
			sprintf( esc_html__( 'The file attempting to be loaded at %s does not exist.', 'ddwc-multipos' ), $filepath )
        );
    }
}
