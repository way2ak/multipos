<?php
/**
 * API\Includes\Reports\DataStore class file.
 *
 * @package MultiPOS - Point of Sale for WCFM Marketplace
 * @version 1.0.0
 */

namespace DDWCMultiPOS\Api\Includes\Reports;
...
/**
 * API\Includes\Reports\DataStore: Common parent for custom report data stores.
 */
class DDWCPOS_Report_DataStore {
	 ...
	 */
	protected function update_intervals_sql_params( &$intervals_query, &$query_args, $db_interval_count, $expected_interval_count, $table_name, $date_column ) {
	...
	protected function cast_numbers( $array ) {
		$retyped_array = [];
		$column_types  = apply_filters( 'woocommerce_rest_ddwcpos_reports_column_types', $this->column_types, $array );
	 ...
	 * @return array
	 */
	protected function get_time_period_sql_params( $query_args, $table_name, $date_column ) {
		$sql_query = [
			'from_clause'       => '',
			'where_time_clause' => '',
		];
		...
		if ( ! empty( $query_args[ 'outlet_id' ] ) ) {
			$sql_query['where_clause'] = " AND postmeta.meta_key = '_ddwcpos_outlet_id' AND postmeta.meta_value=" . $query_args[ 'outlet_id' ];
		} else {
			$user_id = $query_args[ 'user_id' ];
			$user    = get_userdata( $user_id );
			if ( ! empty( $user->roles ) && in_array( 'administrator', $user->roles, true ) ) {
				$sql_query['where_clause'] = " AND postmeta.meta_key = '_ddwcpos_outlet_id'";
			} else {
				$ids = ddwcpos_get_all_outlet_ids_for_sql_by_vendor_id( $user_id );

				$sql_query['where_clause'] = " AND postmeta.meta_key = '_ddwcpos_outlet_id' AND postmeta.meta_value IN ($ids)";
			}
		}
	 ...
	 * @return array
	 */
	protected function get_intervals_sql_params( $query_args, $table_name, $date_column ) {
		$intervals_query = [
			'from_clause'       => '',
			'where_time_clause' => '',
		];
		...
		if ( ! empty( $query_args[ 'outlet_id' ] ) ) {
			$intervals_query['where_clause'] = " AND postmeta.meta_key = '_ddwcpos_outlet_id' AND postmeta.meta_value=" . $query_args[ 'outlet_id' ];
		} else {
			$user_id = $query_args[ 'user_id' ];
			$user    = get_userdata( $user_id );
			if ( ! empty( $user->roles ) && in_array( 'administrator', $user->roles, true ) ) {
				$intervals_query['where_clause'] = " AND postmeta.meta_key = '_ddwcpos_outlet_id'";
			} else {
				$ids = ddwcpos_get_all_outlet_ids_for_sql_by_vendor_id( $user_id );

				$intervals_query['where_clause'] = " AND postmeta.meta_key = '_ddwcpos_outlet_id' AND postmeta.meta_value IN ($ids)";
			}
		}

		$intervals_query = array_merge( $intervals_query, $this->get_time_period_sql_params( $query_args, $table_name, $date_column ) );
