	...
	public static function rest_validate_between_numeric_arg( $value, $request, $param ) {
		if ( ! wp_is_numeric_array( $value ) ) {
			return new \WP_Error(
				...
				sprintf( esc_html__( '%1$s is not a numerically indexed array.', 'ddwc-multipos' ), $param )
			);
		}
		...
		if (
			...
			return new \WP_Error(
				...
				sprintf( esc_html__( '%s must contain 2 numbers.', 'ddwc-multipos' ), $param )
			);
		}
	...
	public static function rest_validate_between_date_arg( $value, $request, $param ) {
		...
		if (
			...
			return new \WP_Error(
				'rest_invalid_param',
				/* translators: %s: parameter name */
				sprintf( esc_html__( '%s must contain 2 valid dates.', 'ddwc-multipos' ), $param )
			);
		}
