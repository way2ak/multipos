<?php
/**
 * API\Includes\Reports\Products\DataStore class file.
 *
 * @package MultiPOS - Point of Sale for WCFM Marketplace
 * @version 1.0.0
 */

namespace DDWCMultiPOS\Api\Includes\Reports\Products;
...
/**
 * API\Includes\Reports\Products\DataStore: Common parent for custom report data stores.
 */
class DDWCPOS_Products_DataStore extends DDWCPOS_Report_DataStore {
	...
	public static function init() {
		add_action( 'ddwcpos_reports_delete_order_stats', [ __CLASS__, 'sync_on_order_delete' ], 10 );
	}
		...
		foreach ( $products_data as $key => $product_data ) {
			...
			if ( $query_args['extended_info'] ) {
				...
				if ( ! $product ) {
					...
					$products_data[ $key ]['extended_info']['name'] = $product_names[ $product_id ] ? sprintf( esc_html__( '%s (Deleted)', 'ddwc-multipos' ), $product_names[ $product_id ] ) : esc_html__( '(Deleted)', 'ddwc-multipos' );
					continue;
				}

				$extended_attributes = apply_filters( 'ddwcpos_rest_reports_products_extended_attributes', $this->extended_attributes, $product_data );
	...
	public static function sync_order_products( $order_id ) {
		...
		foreach ( $order_items as $order_item ) {
			 ...
			 * @param int $order_id      Order ID.
			 */
			do_action( 'ddwcpos_reports_update_product', $order_item_id, $order->get_id() );
	...
	public static function sync_on_order_delete( $order_id ) {
		 ...
		 * @param int $order_id   Order ID.
		 */
		do_action( 'ddwcpos_reports_delete_product', 0, $order_id );
	}
