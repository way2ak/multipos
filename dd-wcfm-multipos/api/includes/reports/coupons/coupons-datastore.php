<?php
/**
 * API\Includes\Reports\Coupons\DataStore class file.
 *
 * @package MultiPOS - Point of Sale for WCFM Marketplace
 * @version 1.0.0
 */

namespace DDWCMultiPOS\Api\Includes\Reports\Coupons;
...
/**
 * API\Includes\Reports\Coupons\DataStore: Coupon Datastore
 */
class DDWCPOS_Coupons_DataStore extends DDWCPOS_Report_DataStore {
	...
	public static function init() {
		add_action( 'ddwcpos_reports_delete_order_stats', [ __CLASS__, 'sync_on_order_delete' ], 5 );
		add_action( 'delete_post', [ __CLASS__, 'delete_coupon' ] );
	}
	...
	public function get_data( $query_args ) {
		...
		if ( false === $data ) {
			...
			if ( count( $included_coupons ) > 0 ) {
				...
				$join_selections = $this->format_join_selections( $fields, [ 'coupon_id' ] );
	...
	public static function sync_order_coupons( $order_id ) {
		...
		foreach ( $coupon_items as $coupon_item ) {
			 ...
			 * @param int $order_id  Order ID.
			 */
			do_action( 'ddwcpos_reports_update_coupon', $coupon_id, $order_id );
	...
	public static function sync_on_order_delete( $order_id ) {
		 ...
		 * @param int $order_id  Order ID.
		 */
		do_action( 'ddwcpos_reports_delete_coupon', 0, $order_id );
	}
